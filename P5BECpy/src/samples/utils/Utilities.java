...
public class Utilities {
  ...
  public static boolean validateResponseNotNull(Object response, String responseObjectName)
          throws OffAmazonPaymentsServiceException {
    System.out.println("validate  response util" + response);
    if (response != null) {
      return true;
    } else {
  ...
  /**
   * Prints to System.out the key on the first line and the value on
   * the second, preceded by one tab. Both lines are preceded by
   * tabLevel tabs.
   *
   * @param key
   * @param value
  ...
  /**
   * Prints to outStream the key on the first line and the value on
   * the second, preceded by one tab. Both lines are preceded by
   * tabLevel tabs.
   *
   * @param key
